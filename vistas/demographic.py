...
def show(data, filters, colors):
    ...
    with col2:
        if "sexo_" in data["fiebre"].columns and "edad_" in data["fiebre"].columns:
            ...
            df_edad_sexo = df_edad_sexo[
                ~df_edad_sexo["grupo_edad"].isin(["No especificado"])
            ]
            ...
            if (set(sexo_edad.index) - set(["Total"])).issubset(set(orden_grupos)):
                ordered_index = [
                    ...
                    if g in sexo_edad.index and g != "No especificado"
                ]
            ...
            sexo_edad_plot = pd.crosstab(
                df_edad_sexo["grupo_edad"], df_edad_sexo["sexo_"]
            )

            # Convertir a formato largo para Plotly
            sexo_edad_long = sexo_edad_plot.reset_index().melt(
                id_vars="grupo_edad", var_name="Sexo", value_name="Cantidad"
            )
            ...
            if set(sexo_edad_long["grupo_edad"]).issubset(set(orden_grupos)):
                sexo_edad_long["grupo_edad"] = pd.Categorical(
                    ...
                    categories=[
                        g
                        for g in orden_grupos
                        if g in sexo_edad_long["grupo_edad"].unique()
                    ],
                    ordered=True,
                )
            ...
            st.warning(
                "No se encontraron datos suficientes para el análisis por sexo y edad."
            )
    ...
    if "per_etn_" in data["fiebre"].columns:
        ...
        # Aplicar mapeo
        df_etnia["etnia_desc"] = (
            df_etnia["per_etn_"].map(etnia_mapping).fillna("No especificado")
        )
    ...
    grupos_existentes = [
        col for col in grupos_especiales.keys() if col in data["fiebre"].columns
    ]
    ...
    if grupos_existentes:
                    ...
                    pd.DataFrame(
                        {
                            ...
                            "Total": [total],
                            "Porcentaje Sí": [
                                si_count / total * 100 if total > 0 else 0
                            ],
                        }
                    ),
                ]
        ...
        if len(grupos_df) > 0:
            ...
            st.warning(
                "No se encontraron datos disponibles para poblaciones especiales."
            )
    else:
        st.warning(
            "No se encontraron columnas de grupos poblacionales especiales en los datos."
        )
    ...
    if "gp_gestan" in data["fiebre"].columns and "sem_ges_" in data["fiebre"].columns:
        ...
        if len(gestantes_df) > 0:
            ...
            with col1:
                create_stat_box(
                    "Total Gestantes", f"{len(gestantes_df)}", colors["primary"]
                )

            with col2:
                create_stat_box(
                    "Promedio Semanas", f"{sem_mean:.1f}", colors["secondary"]
                )

            with col3:
                create_stat_box(
                    "Mediana Semanas", f"{sem_median:.0f}", colors["accent"]
                )

            with col4:
                create_stat_box(
                    "Rango Semanas", f"{sem_min:.0f} - {sem_max:.0f}", colors["primary"]
                )
