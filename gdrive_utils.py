...
class GoogleDriveManager:
    ...
    def download_file(self, file_id, file_name, timeout=30):
        ...
        try:
            ...
            if os.path.exists(cache_path) and os.path.getsize(cache_path) > 0:
                logger.info(f"✅ Descargado: {file_name} ({os.path.getsize(cache_path)} bytes)")
                return cache_path
            else:
    ...
    def download_shapefiles(self):
        ...
        if success_count >= 4:  # Al menos municipios completos
            logger.info(f"✅ Shapefiles descargados: {success_count}/{len(shapefile_files)}")
            return {"files": downloaded_files, "cache_dir": self.cache_dir}
        else:
            logger.error(f"❌ Shapefiles insuficientes: {success_count}/{len(shapefile_files)}")
            return None
...
def process_data_optimized(casos_df, epizootias_df):
    ...
    try:
        ...
        casos_df = casos_df.rename(columns={k: v for k, v in casos_map.items() if k in casos_df.columns})
        epizootias_df = epizootias_df.rename(columns={k: v for k, v in epizootias_map.items() if k in epizootias_df.columns})
